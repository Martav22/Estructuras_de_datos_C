...
/*
*Nombre del modulo: graph_opcional.c
...
*Fecha: 17/02/2019
*Modulos usados: graph_opcional.h, stack_fp.h, 
*Comentarios: 
  Ultima modificacion: 16/03/2019, se realizaron varias funciones:
  la de busqueda en profundidad, y al que imprime el camino
...
int node_print_AntecesorRecur(FILE *pf, Node *n, Graph *g);

/*Busca el nodo con el Id pasado
 como argumento y devuelve la 
 posicion del mismo en el
...
/*Se inicializa un grafo, poniendo
 a NULL las direcciones de cada 
 nodo del array, y a 0
...
/*Se inserta un nodo en caso de que este
 no este en el grafo*/
...
/*En esta funcion se actualiza el estado
 del nodo que tiene el mismo Id que n y
 se pone la informacion de n*/
...
/*En esta funcion se pasa un Id, 
y se busca las conexiones, 
...
con las conexiones que tiene el nodo
 en la posicion del array pasada como
argumento*/
int* graph_getConectionsIndex(Graph * g, int index) {
...
/*Esta funcion devuelve la direccion 
de un array con los ids de los nodos
que estan conectados al nodo 
que tiene como Id, el pasado 
como argumento*/
...
[nombre del nodo, Id, Conexiones] 
 Ids de los nodo con
los que tiene conexion*/
int graph_print(FILE *pf, Graph * g) {
    int i, j, retorno=0, *array, num;
    char *name;
...
/*Sirve para leer un grafo desde un archivo de texto*/
...
/*Imprime el camino qu tiene como destino
el nodo pasado por argumento, se usa la 
recursividad*/
int node_print_AntecesorRecur(FILE *pf, Node *n, Graph *g){
  int num, id;
  Node *antecesor = NULL;
  ...
  id = node_getAntecesor(n);
  if(id == -1){
    fprintf(stderr, "%s\n",strerror(errno) );
    return 0;
  }
  
  antecesor = graph_getNode(g, id);

  
  if(antecesor != NULL){
    num =+ node_print_AntecesorRecur(pf, antecesor, g);
  }

  num =+ node_print(pf, n);
  fprintf(pf, "\n" );
  node_destroy(antecesor);
    ...
    while( stack_isEmpty(s) == FALSE){
        ...
        if( label == BLANCO){
          ...
          for(i = 0; i < numconex ; i++){
            id_vecino = arrayids[i];

            vecino = graph_getNode (g, id_vecino);
            if(!vecino){
              stack_destroy(s);
              node_destroy(n1);
              node_destroy(n);
            ...
            if (node_setAntecesor(vecino, id) ==  NULL){
              stack_destroy(s);
              node_destroy(n1);
              node_destroy(n);
              free(arrayids);
